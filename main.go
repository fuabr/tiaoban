...
import (
	"flag"
	"fmt"
	"github.com/fuabr/tiaoban/utils/stop"
	"io"
	"math/rand"
	"net/http"
	"os"
...
func init() {
	...
	if !utils.CheckQuestionDB() {
		...
		//log.Errorln("题库文件不存在或已损坏，请手动前往 https://github.com/fuabr/tiaoban/blob/main/conf/QuestionBank.db 下载并放入程序根目录")
	}
}
